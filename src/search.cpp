...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, Depth residualReduction) {
        ...
        && ss->ply >= thisThread->nmpMinPly && !is_loss(beta))
    {
        ...
        Value nullValue = -search<NonPV>(pos, ss + 1, -beta, -beta + 1, depth - R, false,0);
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        Depth r = reduction(improving, depth, moveCount, delta) + residualReduction;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
                ...
                && (ttData.bound & BOUND_LOWER) && ttData.depth >= depth - 3)
            {
                ...
                value =
                  search<NonPV>(pos, ss, singularBeta - 1, singularBeta, singularDepth, cutNode,0);
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            value         = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha, d, true, r%1024);
            ss->reduction = 0;
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > 3495) - (r > 5510 && newDepth > 2), !cutNode,0);
        }
