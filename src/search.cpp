...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    return 6991 * pcv + 6658 * micv + 7560 * (wnpcv + bnpcv) + 6352 * cntcv;
}
...
int reduction_correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    return 7014 * pcv + 6695 * micv + 7714 * (wnpcv + bnpcv) + 6274 * cntcv;
}
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        r -= std::max(RcorrectionValue/34855,-RcorrectionValue/34124);
