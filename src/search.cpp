...
moves_loop:  // When in check, search starts here
    ...
    MovePicker mp(pos, ttData.move, depth, &thisThread->mainHistory, &thisThread->lowPlyHistory,
                  &thisThread->captureHistory, contHist, &thisThread->pawnHistory,&thisThread->knightHistory, ss->ply);
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        if (type_of(pos.piece_on(prevSq)) == KNIGHT)
            thisThread->knightHistory[~us][prevSq][knight_attack_index(pos.pieces(us),prevSq)] << scaledBonus * 362 / 32768;
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    if (type_of(pos.moved_piece(move)) == KNIGHT)
        workerThread.knightHistory[us][move.to_sq()][knight_attack_index(pos.pieces(~us),move.to_sq())] << bonus * 1067 / 1024;
