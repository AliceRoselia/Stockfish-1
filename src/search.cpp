...
TUNE(SetRange(0,3000),captureMultiplier,quietMultiplier);
...
void update_all_stats(const Position& pos,
    ...
    if (!pos.capture_stage(bestMove))
    {
        ...
        for (Move move : quietsSearched)
            update_quiet_histories(pos, ss, workerThread, move, -malus * (quietBase-(quietSearchedNum++))*quietMultiplier / 32768);
    }
    else
    {
    ...
    for (Move move : capturesSearched)
    {
        ...
        captureHistory[movedPiece][move.to_sq()][capturedPiece] << -malus * (captureBase - (captureSearchedNum++)) * captureMultiplier / 32768;
    }
}
