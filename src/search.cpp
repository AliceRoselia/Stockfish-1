...
moves_loop:  // When in check, search starts here
    ...
    MovePicker mp(pos, ttData.move, depth, &thisThread->mainHistory, &thisThread->lowPlyHistory,
                  &thisThread->captureHistory, contHist, &thisThread->pawnHistory,&thisThread->evasionHistory, ss->ply);
...
void update_all_stats(const Position&      pos,
    ...
    if (!pos.capture_stage(bestMove))
    {
        if (ss->inCheck)
        {
            EvasionPieceToHistory& evasionHistory = workerThread.evasionHistory;
            Color us = pos.side_to_move();
            Square ksq = pos.square<KING>(us);
            evasionHistory[moved_piece][bestMove.to_sq()][ksq]<<bonus;
            for (Move move:quietsSearched)
                evasionHistory[pos.moved_piece(move)][move.to_sq()][ksq]<<-malus;
        }
