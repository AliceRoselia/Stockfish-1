...
moves_loop:  // When in check, search starts here
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        const int scaledBonus = std::min(129 * depth - 77, 1387) * bonusScale;
...
void update_all_stats(const Position& pos,
    ...
    int bonus =
      std::min(101 * depth - 91, 1537) + 347 * (bestMove == ttMove) + (ss - 1)->statScore / 32;
    int malus = std::min(805 * depth - 243, 2490) - 17 * moveCount;
