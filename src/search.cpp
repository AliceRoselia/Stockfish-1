...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    workerThread.boardColorHistory[~us][WHITE][board_color_key(pos,~us,WHITE)][type_of(pos.moved_piece(move))][move.to_sq()] << (bonus > 0 ? 1 : 2)*bonus;
    workerThread.boardColorHistory[~us][BLACK][board_color_key(pos,~us,BLACK)][type_of(pos.moved_piece(move))][move.to_sq()] << (bonus > 0 ? 1 : 2)*bonus;
