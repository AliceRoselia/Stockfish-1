...
ExtMove* MovePicker::score(MoveList<Type>& ml) {
    ...
    for (auto move : ml)
    {
        ...
        else if constexpr (Type == QUIETS)
        {
            ...
            m.value = 3851 * (*mainHistory)[us][m.raw()];
            m.value += 2148 * (*pawnHistory)[pawn_history_index(pos)][pc][to];
            m.value += 1587 * (*continuationHistory[0])[pc][to];
            m.value += 1052 * (*continuationHistory[1])[pc][to];
            m.value += 280 * (*continuationHistory[2])[pc][to];
            m.value += 328 * (*continuationHistory[3])[pc][to];
            //m.value -= 34 * (*continuationHistory[4])[pc][to];
            m.value += 187 * (*continuationHistory[5])[pc][to];
            m.value += (bool(pos.check_squares(pt) & to) && pos.see_ge(m, -75)) * 28835489;
            m.value += threatByLesser[pt] & to ? -25465757 : 26806060 * bool(threatByLesser[pt] & from);

            if (ply < LOW_PLY_HISTORY_SIZE)
                m.value += 6112 * (*lowPlyHistory)[ply][m.raw()] / (1 + ply);
            //m.value /= 1024;
