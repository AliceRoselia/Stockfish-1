...
inline int material_imbalance(const Position& pos, Color side){
    int material = (pos.count<PAWN>(side) + pos.count<KNIGHT>(side)*3 + pos.count<BISHOP>(side)*4 + pos.count<ROOK>(side)*6 + pos.count<QUEEN>(side)*13 -
                    pos.count<PAWN>(~side) + pos.count<KNIGHT>(~side)*3 + pos.count<BISHOP>(~side)*4 + pos.count<ROOK>(~side)*6 + pos.count<QUEEN>(~side)*13
    );
