...
void Position::set_state() const {
    ...
    for (Piece pc : Pieces)
        for (int cnt = 0; cnt < piece_count(pc); ++cnt)
...
DirtyPiece Position::do_move(Move                      m,
    ...
    else if (captured)
    {
        ...
        st->materialKey ^= Zobrist::psq[captured][8 + piece_count(captured)];
    ...
    if (type_of(pc) == PAWN)
    {
        ...
        else if (m.type_of() == PROMOTION)
        {
            ...
            st->materialKey ^= Zobrist::psq[promotion][8 + piece_count(promotion) - 1]
                             ^ Zobrist::psq[pc][8 + piece_count(pc)];
...
bool Position::pos_is_ok() const {
    ...
    if (piece_count(W_KING) != 1 || piece_count(B_KING) != 1
        || attackers_to_exist(square<KING>(~sideToMove), pieces(), sideToMove))
        assert(0 && "pos_is_ok: Kings");

    if ((pieces(PAWN) & (Rank1BB | Rank8BB)) || piece_count(W_PAWN) > 8 || piece_count(B_PAWN) > 8)
    ...
    for (Piece pc : Pieces)
        if (piece_count(pc) != popcount(pieces(color_of(pc), type_of(pc)))
            || piece_count(pc) != std::count(board, board + SQUARE_NB, pc))
